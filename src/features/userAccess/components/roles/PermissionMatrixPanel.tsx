...
import type { PermissionCategory, PermissionDefinition, RoleChangeLog, RoleDefinition } from '../../types'
...
function PermissionMatrixPanel({
  ...
  return (
    ...
    >
      {mode ? (
        <div className="mb-5 rounded-[1.2rem] border border-slate-200 bg-[linear-gradient(180deg,#f8fbff_0%,#eef4fb_100%)] p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold text-slate-900">{mode === 'create' ? 'Create Permission' : 'Edit Permission'}</h3>
              <p className="mt-1 text-sm text-slate-500">Manage permission catalog entries used by the RBAC matrix.</p>
            </div>
            <button onClick={() => { setMode(null); setDraft(emptyDraft) }} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700">Cancel</button>
          </div>
          <div className="mt-4 grid gap-4 min-[1700px]:grid-cols-2">
            <label className="text-sm text-slate-600"><span className="mb-2 block font-medium">Permission Name</span><input value={draft.label} onChange={(e) => setDraft((p) => ({ ...p, label: e.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-slate-800 outline-none" /></label>
            <label className="text-sm text-slate-600"><span className="mb-2 block font-medium">Permission Key</span><input value={draft.key} onChange={(e) => setDraft((p) => ({ ...p, key: e.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-slate-800 outline-none" /></label>
            <label className="text-sm text-slate-600"><span className="mb-2 block font-medium">Category</span><select value={draft.category} onChange={(e) => setDraft((p) => ({ ...p, category: e.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-slate-800 outline-none">{permissionCatalog.map((category) => <option key={category.category} value={category.category}>{category.category}</option>)}</select></label>
            <label className="text-sm text-slate-600"><span className="mb-2 block font-medium">Description</span><input value={draft.description} onChange={(e) => setDraft((p) => ({ ...p, description: e.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-slate-800 outline-none" /></label>
          </div>
      ...
      <div className="space-y-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search permissions, labels, or descriptions" className="w-full rounded-xl border border-[rgba(120,142,179,0.35)] bg-white px-4 py-3 text-sm text-slate-700 outline-none lg:max-w-md" />
        ...
        <div className="rounded-[1.2rem] border border-[rgba(120,142,179,0.35)] bg-[linear-gradient(180deg,#f7faff_0%,#ecf2fb_100%)] p-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select value={copyFromRoleId} onChange={(e) => setCopyFromRoleId(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none">
            ...
            }} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700">Copy Permissions From Role</button>
            <button onClick={() => applyBulk((permission) => selectedRole.defaultPermissions.includes(permission) && !getPermissionBlockReason(selectedRole.id, permission), 'Reset selected role permissions to default baseline.')} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700">Reset To Default</button>
          </div>
          {selectedRole ? <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-slate-500"><Badge label={`${selectedRole.level}`} tone="info" /><span>Priority {selectedRole.priority}</span><span>Coverage {selectedRole.coverage}%</span></div> : null}
        </div>
        ...
        <div className="space-y-3 min-[1650px]:pr-2">
          {visibleCategories.map((category) => {
            ...
            return (
              <div key={category.category} className="rounded-[1.1rem] border border-slate-200 bg-white">
                <button onClick={() => setCollapsedGroups((previous) => ({ ...previous, [category.category]: !previous[category.category] }))} className="flex w-full items-center justify-between bg-[#dce7f6] px-4 py-3 text-left text-sm font-semibold text-[#243a63]">
                    ...
                    {category.permissions.map((permission) => (
                      <div key={permission.key} className="flex flex-col gap-3 px-4 py-3 md:flex-row md:items-center md:justify-between">
                        <div title={permission.description}>
                          ...
                          <p className="text-xs text-slate-500">{permission.label}</p>
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => openEditPermission(permission)} className="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-700">Edit Permission</button>
        ...
        <div className="overflow-x-auto rounded-[1.2rem] border border-[rgba(120,142,179,0.35)]">
          <table className="min-w-[980px] border-separate border-spacing-0">
            <thead className="sticky top-0 z-20">
              <tr>
                <th className="sticky left-0 z-30 border-b border-[#445d8c] bg-[#0f2448] px-4 py-4 text-left text-xs uppercase tracking-[0.18em] text-white">Permission</th>
                {sortedRoles.map((role) => (
                  <th key={role.id} className="border-b border-[#445d8c] bg-[#0f2448] px-4 py-4 text-left text-xs uppercase tracking-[0.18em] text-white">
                    <div className="space-y-1"><span>{role.name}</span><div className="h-1.5 rounded-full bg-[#2a3f67]"><div className="h-1.5 rounded-full bg-[#4f7de1]" style={{ width: `${role.coverage}%` }} /></div></div>
                  </th>
                ))}
              </tr>
            </thead>
              ...
              {visibleCategories.map((category) => (
                <>
                  <tr key={category.category}>
                    <td colSpan={sortedRoles.length + 1} className="bg-[#dce7f6] px-4 py-3 text-sm font-semibold text-[#243a63]">{category.category}</td>
                  </tr>
                  {category.permissions.map((permission) => (
                    <tr key={permission.key} className="odd:bg-white even:bg-[#f5f8fd]">
                      ...
                      {sortedRoles.map((role) => {
                        ...
                        return (
                          <td key={`${role.id}-${permission.key}`} className="border-b border-slate-200 px-4 py-3 text-sm">
                            <div className="space-y-2">
                              ...
                              <p className={`max-w-[12rem] text-xs ${blockReason ? 'text-amber-600' : 'text-slate-500'}`}>{blockReason ? `Locked: ${blockReason}` : permission.description}</p>
                            </div>
                          </td>
                        )
                      })}
      ...
      {pendingDeletePermission ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/35 px-4">
          <div className="w-full max-w-md rounded-[1.35rem] bg-white p-6 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-slate-500">Delete <span className="font-semibold text-slate-700">{pendingDeletePermission.key}</span>? This will remove it from the RBAC catalog.</p>
            <div className="mt-5 flex justify-end gap-3">
              <button onClick={() => setPendingDeletePermission(null)} className="rounded-xl bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700">Cancel</button>
