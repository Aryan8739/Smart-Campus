      ...
      <section className="grid gap-7 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <DataPanel title="Approval Queue">
            ...
            {['Pending', 'Approved', 'Rejected'].map((value) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-semibold ${
                  filters.approval === value ? 'bg-[rgb(var(--color-primary))] text-white' : 'bg-slate-100 text-slate-600'
                }`}
              >
                {value}
              </button>
          ...
          <div className="space-y-4">
            ...
            {can('ROLE_EDIT') ? (
              <div className="rounded-[1.1rem] bg-amber-50 p-4 text-sm text-amber-700">
