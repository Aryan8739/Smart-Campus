...
import type { DepartmentTicket } from '../types'
  ...
  return (
    <section className="space-y-5">
      <header className="rounded-3xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-card))] p-6 shadow-sm">
        <h2 className="text-xl font-semibold">SLA Risk Command Desk</h2>
        <p className="mt-1 text-sm text-[rgb(var(--color-text-secondary))]">
          Early warning visibility and escalation controls for service-level compliance.
        </p>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <article className="rounded-2xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-card))] p-4">
          <p className="text-xs uppercase tracking-wide text-[rgb(var(--color-text-secondary))]">Escalated</p>
          <p className="mt-1 text-2xl font-bold text-[rgb(var(--color-warning))]">{tickets.filter((ticket) => ticket.status === 'Escalated').length}</p>
        </article>
        <article className="rounded-2xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-card))] p-4">
          <p className="text-xs uppercase tracking-wide text-[rgb(var(--color-text-secondary))]">Protected</p>
        ...
        <div className="mt-3 space-y-2.5">
          ...
          {riskTickets.length === 0 ? (
            <p className="rounded-xl border border-dashed border-[rgb(var(--color-border))] bg-[rgb(var(--color-bg))] p-3 text-xs text-[rgb(var(--color-text-secondary))]">
              No ticket currently in selected risk band.
            </p>
          ) : null}
        </div>
      </article>
