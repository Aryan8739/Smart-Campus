...
import type { DepartmentNotification } from '../types'
...
function NotificationsTab({ notifications, onMarkRead, onMarkAllRead, onClearRead }: NotificationsTabProps) {
  const [severityFilter, setSeverityFilter] = useState<'all' | DepartmentNotification['severity']>('all')
  ...
  return (
    <section className="space-y-5">
      <header className="rounded-3xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-card))] p-6 shadow-sm">
        <h2 className="text-xl font-semibold">Department Alerts</h2>
        <p className="mt-1 text-sm text-[rgb(var(--color-text-secondary))]">
          Intake actions, budget warnings, and service performance signals.
        </p>

        <div className="mt-3 flex flex-wrap gap-2">
          <select
            value={severityFilter}
            onChange={(event) => setSeverityFilter(event.target.value as 'all' | DepartmentNotification['severity'])}
